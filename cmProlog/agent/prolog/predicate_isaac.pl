...
:- module(predicate,
    ...
    currentObjectVertex_old/2,
    currentRobotBodyVertex/2,
    currentRobotBodyXY/2,
    currentHandVertex/2,
    currentHandVertex_old/2,
...
batteryState(State):-
	%nb_current(predicate__batteryState_low_robot_battery_threshold, Low_robot_battery_threshold),
  %nb_current(predicate__batteryState_high_robot_battery_threshold, High_robot_battery_threshold),
...
objectSize(Object, Size):-
   ...
   not(rdfs_individual_of(Object,  knowrob:'Robot')),
   %not(rdf(Object, rdf:type,  knowrob:'Hand')),
   rdf(Object, knowrob:depthOfObject, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
   rdf(Object, knowrob:widthOfObject, literal(type(_,S1y))),atom_to_term(S1y,S1Y,_),
   rdf(Object, knowrob:heightOfObject, literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
...
locatedAt(Object, Location) :-
   (currentObjectPerception(Object, CurrentPerception);currentHandPerception(Object, CurrentPerception);currentRobotBodyPerception(Object, CurrentPerception)),
...
robotBodySize(Robot, RobotBodySize):-
    rdfs_individual_of(Robot,  knowrob:'Robot'),
    rdf(Robot, knowrob:depthOfObject, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
    rdf(Robot, knowrob:widthOfObject, literal(type(_,S1y))),atom_to_term(S1y,S1Y,_),
    rdf(Robot, knowrob:heightOfObject, literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
    RobotBodySize = [S1X, S1Y, S1Z].
...
tilted_Object(Object):-
   %nb_current(predicate__tilted_Object_low_angle_threshold, Low_angle_threshold),
   %nb_current(predicate__tilted_Object_high_angle_threshold, High_angle_threshold),
   currentObjectPose(Object, Pose),
...
standing_Object(Object):-
   %nb_current(predicate__standing_Object_low_angle_threshold, Low_angle_threshold),
   %nb_current(predicate__standing_Object_high_angle_threshold, High_angle_threshold),
   currentObjectPose(Object, Pose),
   nth0(3, Pose, Tilted_RadiusX),
   nth0(4, Pose, Tilted_RadiusY),
   once(
...
currentObjectVertex_old(Object, Vertice):-
   currentObjectPose(Object, ObjectPose),
   objectSize(Object, ObjectSize),
   vertex_old(ObjectPose, ObjectSize, Vertice).
...
%추가 및 수정
currentRobotBodyVertex(Robot, Vertice):-
   currentRobotBodyPose(Robot, RobotBodyPose),
   robotBodySize(Robot, RobotBodySize),
   vertex(RobotBodyPose, RobotBodySize, Vertice).
...
currentHandVertex_old(Hand, Vertice):-
   currentHandPose(Hand, HandPose),
   handSize(Hand, HandSize),
   vertex_old(HandPose, HandSize, Vertice).
...
vertex_old(Pose, Size, Vertice):-
   nth0(0, Pose, ObjectPoseX),
   nth0(1, Pose, ObjectPoseY),
   nth0(2, Pose, ObjectPoseZ),


   nth0(0, Size, ObjectSizeX),
   nth0(1, Size, ObjectSizeY),
   nth0(2, Size, ObjectSizeZ),
...
intersects3(Object, IntersectVolume):-
   ...
   currentObjectVertex_old(Object, ObjectPoint),
   currentHandVertex_old(Hand, SpacePoint),
...
opened_hand(Hand):-
   %nb_current(predicate__joint_opened_hand_finger_number, Finger_number),
   joint_check(Hand,Number),
   Number >= 2.
   
closed_hand(Hand):-
   %nb_current(predicate__joint_closed_hand_finger_number, Finger_number),
   joint_check(Hand,Number),
   Number < 2.
...
joint_check(Hand, Number):-
   %nb_current(predicate__joint_check_joint_angle_threshold, Joint_threshold),
   rdfs_individual_of(Hand,  knowrob:'Hand'),
...
detected_object(Object) :-
    %nb_current(predicate__detected_object_min_time_threshold, Min_time_threshold),
    %nb_current(predicate__detected_object_max_time_threshold, Max_time_threshold),
...
isLifting(Robot, Joint):-
    rdfs_individual_of(Robot, knowrob: 'Robot'),
     not(rdfs_individual_of(Robot, knowrob: 'Hand')),
     rdfs_individual_of(Joint,  srdl2comp:'Lifter'),
    %rdfs_individual_of(Joint, 'http://knowrob.org/kb/srdl2-comp.owl#Lifter')
    currentJointAngle(Joint, Angle),
    Angle > 0.003.
  
isLowering(Robot, Joint):-
    rdfs_individual_of(Robot, knowrob: 'Robot'),
     not(rdfs_individual_of(Robot, knowrob: 'Hand')),
     rdfs_individual_of(Joint,  srdl2comp:'Lifter'),
    %rdfs_individual_of(Joint, 'http://knowrob.org/kb/srdl2-comp.owl#Lifter')
...
isMoving(Robot, Joint):-
    rdfs_individual_of(Robot, knowrob: 'Robot'),
    not(rdfs_individual_of(Robot, knowrob: 'Hand')),
    rdfs_individual_of(Joint,  srdl2comp:'R_wheelJoint'),
    %rdfs_individual_of(Joint, 'http://knowrob.org/kb/srdl2-comp.owl#Lifter')
    currentJointVelocity(Joint, Vel),
    abs(Vel) >= 0.00001. %0.1

isCenterOf(Robot, Rack):-
	   rdfs_individual_of(Robot, knowrob: 'Robot'),
      not(rdfs_individual_of(Robot, knowrob: 'Hand')),
   	rdfs_individual_of(Rack, knowrob: 'Rack'),
   	currentRobotBodyPose(Robot, Pose1),
   	[P1X, P1Y, P1Z | _] = Pose1,
   	%currentObjectPose(Rack, Pose2),
      rdf(Rack, knowrob:xCoord, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
      rdf(Rack, knowrob:yCoord, literal(type(_,S1y))),atom_to_term(S1y,S1Y,_),
      rdf(Rack, knowrob:zCoord, literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
   	%[S1X, S1Y, S1Z | _] = Pose2,
   	Distance = sqrt((P1X-S1X)**2 + (P1Y-S1Y)**2),
      %Distance = P1X-S1X,
	   Distance < 3,
      rdfs_individual_of(Joint,  srdl2comp:'R_wheelJoint'),
      not(isMoving(Robot, Joint)).

mountedBy(Rack, Robot):-
	   isCenterOf(Robot, Rack),
      isLifting(Robot, Joint).
   

ableToLift(Robot, Rack):-
      rdfs_individual_of(Robot,  knowrob:'Robot'),
      not(rdfs_individual_of(Robot, knowrob: 'Hand')),
      rdfs_individual_of(Rack,  knowrob:'Rack'),
      isCenterOf(Robot, Rack),
      rdf(Robot, knowrob:massOfObject, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
      rdf(Rack, knowrob:massOfObject, literal(type(_,P1x))),atom_to_term(P1x,P1X,_),
      S1X > P1X.

atLoadingStation(Rack, Station):-
      rdfs_individual_of(Rack, knowrob: 'Rack'),
      rdfs_individual_of(Station, arbi: 'LStation'),
      rdf(Station, knowrob:xCoord, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
      rdf(Station, knowrob:yCoord, literal(type(_,S1y))),atom_to_term(S1y,S1Y,_),
      rdf(Station, knowrob:zCoord, literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
...
rackOn(Rack, Station):-
      ...
      rdf(Station, knowrob:vertexnum, literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
...
emptyStoringStation(Station):-
      ...
      findall(Object,  (rdfs_individual_of(Object, knowrob: 'Rack')), Objects),
...
cargoOn(Box, Rack):-
      rdfs_individual_of(Box, knowrob: 'Box'),
      rdfs_individual_of(Rack, knowrob: 'Pallet'),
      currentObjectPose(Box, Pose1),
      currentObjectPose(Rack, Pose2),
      [P1X, P1Y | _] = Pose1,
      [S1X, S1Y | _] = Pose2,
      P1X == S1X,
...
moveTypeOut(Station, MoveType):-
      rdfs_individual_of(Station, arbi: 'Station'),
      rdf(Station, knowrob:moveTypeOut, literal(type(_,P1x))),atom_to_term(P1x,P1X,_),
      %rdf(Station, knowrob:preparationVertex, literal(type(_,V1x))),atom_to_term(V1x,V1X,_),
      %append(P1X, V1X, A),
      MoveType = P1X.
   
moveType(Station, InOut, Result):-
      InOut == 'in' -> moveTypeIn(Station, MoveType), Result = MoveType;
      moveTypeOut(Station, MoveType), Result = MoveType.

faceToFace(RobotA, RobotB):-
       rdfs_individual_of(RobotA, knowrob: 'Robot'),
       rdfs_individual_of(RobotB, knowrob: 'Robot'),
       not(RobotA = RobotB),
       currentRobotBodyPerception(RobotA, CurrentPerception1),
       ...
       rdf(Object_Matrix1,'http://knowrob.org/kb/knowrob.owl#m12',literal(type(_,P1z))),atom_to_term(P1z,P1Z,_),
       currentRobotBodyPerception(RobotB, CurrentPerception2),
       ...
       rdf(Object_Matrix2,'http://knowrob.org/kb/knowrob.owl#m12',literal(type(_,S1z))),atom_to_term(S1z,S1Z,_),
       ( abs(P1X-S1X) < Thresh1 ->
         abs(P1Y-S1Y) < Thresh2,
         P1Z / S1Z
       ),
...
hwanSong(RobotA, RobotB, Hwan, Song ):-
       rdfs_individual_of(RobotA, knowrob: 'Robot'),
       rdfs_individual_of(RobotB, knowrob: 'Robot'),
       not(RobotA = RobotB),
       currentRobotBodyPerception(RobotA, CurrentPerception1),
       rdf(CurrentPerception1, knowrob:eventOccursAt, Object_Matrix1),
       rdf(Object_Matrix1,'http://knowrob.org/kb/knowrob.owl#m12',literal(type(_,P1x))),atom_to_term(P1x,P1X,_),
       currentRobotBodyPerception(RobotB, CurrentPerception2),
       rdf(CurrentPerception2, knowrob:eventOccursAt, Object_Matrix2),
       rdf(Object_Matrix2,'http://knowrob.org/kb/knowrob.owl#m12',literal(type(_,S1x))),atom_to_term(S1x,S1X,_),
       Hwan is P1X, Song is S1X.
